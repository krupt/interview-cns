...
class AccountServiceTransferTestIT {
    ...
    fun testTransferDeadlock() {
        ...
        service.submit {
            accountService.transfer(
                TransferOperationRequest(sourceAccountNumber, targetAccountNumber, firstTransferAmount)
            )
        }

        service.submit {
            accountService.transfer(
                TransferOperationRequest(targetAccountNumber, sourceAccountNumber, secondTransferAmount)
            )
        }
        ...
        val (_, sourceAccountBalance) = accountService.get(sourceAccountNumber)
        assertEquals(500.23 - firstTransferAmount + secondTransferAmount, sourceAccountBalance, 0.001)

        val (_, targetAccountBalance) = accountService.get(targetAccountNumber)
        assertEquals(17273.57 + firstTransferAmount - secondTransferAmount, targetAccountBalance, 0.001)
    }
}
